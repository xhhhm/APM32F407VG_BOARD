...
//    lv_style_set_line_width(&transition_dsc,100);
    ...
    lv_style_set_border_width(&style_main, 1);              //黑色部分，向内部一个像素
    lv_style_set_radius(&style_main, LV_RADIUS_CIRCLE);
    lv_style_set_pad_ver(&style_main, -2);                  //*Makes the indicator larger*/
    lv_style_set_size(&style_main,30);                      //长和宽一起设置
    lv_style_set_width(&style_main,80);                     //设置样式宽度
    ...
    lv_slider_set_value(slider, 0,LV_ANIM_OFF);     //设置滑块初始值
    lv_slider_set_range(slider, 0, 1000);            //设置滑块最大最小值

    lv_obj_set_width(slider,50);                    //设置宽度
    lv_obj_set_height(slider, 320);                 //设置高度
    lv_obj_set_size(slider,120, 20);                //设置大小（包括宽高）
    lv_obj_align(slider,LV_ALIGN_CENTER,0,0);      //设置对齐方式

    lv_obj_t *label =lv_label_create(obj);
    lv_obj_align_to(label,slider,LV_ALIGN_OUT_TOP_MID,0,40);//相对另一个对象设置对齐方式
...
void lv_100ask_demo_course_2_2_5(void)
{
    ...
    lv_style_set_bg_opa(&style_obj, 30);                          // 设置颜色透明度

    static lv_style_t style_obj1;

	lv_style_init(&style_obj1);
    lv_style_set_bg_color(&style_obj1, lv_color_hex(0x000000));      // 设置背景色
    lv_style_set_text_color(&style_obj1, lv_color_hex(0xc43e1c));    // 设置文字颜色
//    lv_style_set_bg_color(&style_obj1, lv_color_hex(0xc43e1c));    // 设置背景色
    lv_style_set_bg_opa(&style_obj1, 100);                          // 设置颜色透明度
...
//    lv_obj_add_style(obj2, &style_obj1, LV_STATE_PRESSED); // 按下状态，当对象被按下的时候应用该样式
#endif
